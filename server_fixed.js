...
const dirPath = path.join(__dirname, 'api'); // Path to the lambda functions dir - FIXED
...
const makeLimiterResponseMsg = (retryAfter) => {
  ...
  + 'You can get around these limits by running your own instance of Website Check.';
...
try {
  ...
  files
    ...
    .forEach(dirent => {
      ...
      // Import each API module using dynamic import with proper file URL
      const filePath = path.join(dirPath, dirent.name);
      const fileURL = new URL(`file://${filePath}`).href;
      
      import(fileURL).then(handlerModule => {
...
if (process.env.DISABLE_GUI && process.env.DISABLE_GUI !== 'false') {
  ...
  app.use(async (req, res, next) => {
    ...
    const ssrHandlerURL = new URL(`file://${ssrHandlerPath}`).href;
    
    import(ssrHandlerURL).then(({ handler: ssrHandler }) => {
